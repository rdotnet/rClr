...
# document('~/src/r2clr/packages/rClr')
...
clrInit <- function(debug=FALSE) {
  application_dir_base <- getLibsPath('rClr')
  f <- file.path(application_dir_base, 'ClrFacade.dll')
  f <- path.expand(f)
  stopifnot( file.exists(f) )
  result <- .C("rclr_create_domain", application_dir_base, f, as.integer(debug), PACKAGE=rclr_pkg_name)
}
...
clrShutdown <- function() { # TODO: is this even possible given runtime's constraints?
  result <- .C("rclr_shutdown_clr", PACKAGE=rclr_pkg_name)
}
...
clrLoadAssembly <- function(name) {
  ...
  result <- .C("rclr_load_assembly", name, PACKAGE=rclr_pkg_name)
}
...
#' @export
clrGetInnerPkgName <- function() { rclr_pkg_name }
...
{
  o<-.External("r_create_clr_object", typename, ..., PACKAGE=rclr_pkg_name)
  if (is.null(o)) {
...
clrCall <- function(obj,methodName,...) 
{
  ...
  result <- NULL
  result <-.External(interface, obj@clrobj, methodName, ..., PACKAGE=rclr_pkg_name)
  return(createReturnedObject(result))
}

#' Gets the value of a field or property of an object or class
#'
#' @param obj_or_typename a CLR object, or type name, possibly namespace and assembly qualified type name, e.g. 'My.Namespace.MyClass,MyAssemblyName'.
...
#' }
clrGet <- function(obj_or_typename,name) 
{
  return(clrCallStatic(clrFacadeTypeName, 'GetFieldOrProperty',obj_or_typename, name))
}
...
#' clrCall(f, 'Show')
#' }
clrSet <- function(obj_or_typename, name, value) 
{
  invisible(clrCallStatic(clrFacadeTypeName, 'SetFieldOrProperty',obj_or_typename, name, value))
}
...
#' @export
clrSetEnumProperty <- function(obj_or_typename, name, enumtype, enumval)  
{
  stop('Not yet implemented')
  return(clrCallStatic(reflectionHelperTypeName, 'SetEnumValue',obj_or_typename, name, enumtype, enumval))
}

checkIsExtPtr <- function(ext_ptr) {
  stopifnot("externalptr" %in% class(ext_ptr)) 
}
...
#'
#' @param ext_ptr external pointer to a CLR object (not a cobjRef S4 object)
...
#' }
clrTypeNameExtPtr <- function(ext_ptr) {
  checkIsExtPtr(ext_ptr)
  .External("r_get_typename_externalptr", ext_ptr, PACKAGE=rclr_pkg_name)
}
...
clrTypename <- function(clrobj) {
  name <- .Call("r_get_type_name", clrobj, PACKAGE=rclr_pkg_name)
  name
}
...
#'
#' @return the name of the native library currently loaded: rClr or rClrMono
#' @export
...
{
  ext_ptr <-.External("r_call_static_method", typename, methodName,..., PACKAGE=rclr_pkg_name)
  return(createReturnedObject(ext_ptr))
}
...
#' }
clrGetStaticMembers <- function(obj_or_typename) 
{
  list(Methods=clrGetStaticMethods(obj_or_typename), Fields=clrGetStaticFields(obj_or_typename), Properties=clrGetStaticProperties(obj_or_typename))
}
...
#' @param contains a string that the property names returned must contain
#' @export
clrGetStaticFields <- function( obj_or_typename, contains = '') {
  clrCallStatic(reflectionHelperTypeName, 'GetStaticFields', obj_or_typename, contains)
}
...
#' @export
clrGetStaticProperties <- function( obj_or_typename, contains = '') {
  clrCallStatic(reflectionHelperTypeName, 'GetStaticProperties', obj_or_typename, contains)
}
...
#' @inheritParams clrGetStaticFields
#' @export
clrGetStaticMethods <- function( obj_or_typename, contains = '') {
  clrCallStatic(reflectionHelperTypeName, 'GetStaticMethods', obj_or_typename, contains)
}
...
#'
#' @param ext_ptr the external pointer.
...
#' @import methods
createReturnedObject <- function(ext_ptr) {
  if( is.null(ext_ptr) == TRUE ) {
    return(NULL)
  } else if ("externalptr" %in% class(ext_ptr)) {
    typename <- clrTypeNameExtPtr(ext_ptr)
		return(new("cobjRef", clrobj=ext_ptr, clrtype=typename))
  } else {
    return(ext_ptr)
  }
}
...
getSexpType <- function(sexp) {
  ext_ptr <-.External("r_get_sexp_type", sexp, PACKAGE=rclr_pkg_name)
  return(createReturnedObject(ext_ptr))
}
...
inspectArgs <- function(...) {
  ext_ptr <-.External("r_show_args", ..., PACKAGE=rclr_pkg_name)
  # return(createReturnedObject(ext_ptr))
}
